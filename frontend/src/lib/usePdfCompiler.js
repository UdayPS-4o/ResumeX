...
// ─────────────────────────────────────────────────────────────────────────────
// usePdfCompiler — owns Typst→PDF compilation plus a tiny LRU cache of compiled
...
// instantly instead of after a fresh round-trip + Typst compile.
// ─────────────────────────────────────────────────────────────────────────────
...
const keyFor = (templateId, resume, pageSize, trim, formatting) =>
  `${templateId}|${pageSize}|${trim ? 1 : 0}|${JSON.stringify(formatting || {})}|${JSON.stringify(resume)}`;

export function usePdfCompiler({ resume, templateId, pageSize, trim, formatting, retryNonce, debounceMs = 600 }) {
  ...
  const compileFor = useCallback((tid, r, ps, tr, fmt) => {
    const key = keyFor(tid, r, ps, tr, fmt);
    ...
    const entry = {};
    entry.promise = (async () => {
      const { source, format } = await api.render({ templateId: tid, resume: r, pageSize: ps, formatting: fmt });
  ...
  // of concurrent Typst jobs (that's what was choking the local engine and
  ...
  const prewarm = useCallback((tid) => {
    ...
    const key = keyFor(tid, resume, pageSize, trim, formatting);
    const hit = cache.current.get(key);
    ...
    const runOrSkip = () => {
      ...
      return compileFor(tid, resume, pageSize, trim, formatting).promise;
    };
    const next = prewarmTail.current.then(runOrSkip, runOrSkip);
    prewarmTail.current = next.then(() => {}, () => {}); // keep the chain alive past failures
    return next;
  }, [resume, pageSize, trim, formatting, compileFor]);

  // Track whether we've ever shown a compiled PDF in this hook instance.
  // The very first compile fires immediately — no debounce — so the preview
  // appears as soon as the backend responds rather than after an extra 600ms.
  ...
  useEffect(() => {
    ...
    const hit = cache.current.get(keyFor(templateId, resume, pageSize, trim, formatting));
    ...
    const run = () => {
      setCompiling(true);
      setError(null);
      compileFor(templateId, resume, pageSize, trim, formatting).promise
        .then((url) => {
    ...
    // First-ever compile: fire immediately (no debounce) — the blank preview is
    // the most jarring UX moment and there's nothing to coalesce yet.
    // Subsequent edits: debounce normally so we don't spam the backend.
    const delay = !everShown.current ? 0 : (resumeChanged ? debounceMs : 150);
    if (delay === 0) { run(); return; }
    const timer = setTimeout(run, delay);
    return () => clearTimeout(timer);
  }, [resume, templateId, pageSize, trim, formatting, retryNonce, compileFor, showUrl, debounceMs]);
