    ...
    >
      ...
      <div className="border-t border-slate-100 p-3">
            ...
            {images.map((img, i) => (
              <div key={i} className="relative group">
                ...
                <button
                  ...
                  className="absolute -top-1.5 -right-1.5 w-5 h-5 rounded-full bg-slate-800 text-white text-xs grid place-items-center opacity-0 group-hover:opacity-100 transition"
                  title="Remove"
                >
                  ✕
                </button>
        ...
        <div className="relative rounded-xl border border-slate-200 bg-white shadow-sm focus-within:border-brand-500 focus-within:ring-2 focus-within:ring-brand-100 transition">
          ...
          <button
            ...
            title="Attach image"
            className="absolute bottom-2 right-11 w-8 h-8 grid place-items-center rounded-lg text-slate-400 hover:text-slate-700 hover:bg-slate-100 transition"
          >
          ...
          <button
            ...
            title="Send (Enter)"
            className="absolute bottom-2 right-2 w-8 h-8 grid place-items-center rounded-lg bg-brand-600 text-white hover:bg-brand-700 transition disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed"
          >
      ...
      {dragOver && (
        <div className="absolute inset-2 z-30 rounded-2xl border-2 border-dashed border-brand-400 bg-brand-50/85 backdrop-blur-[1px] grid place-items-center text-center px-6 text-brand-700 text-sm font-semibold pointer-events-none">
          {onImportFile
            ? 'Drop your résumé (PDF, DOCX, TXT) to import — or an image to attach'
...
function Message({ message, onInsert, onUndo, onDismiss, onRetry, onOptimize, sending, messageIndex }) {
        ...
        <div className={`max-w-[85%] rounded-2xl px-3.5 py-2.5 text-sm leading-relaxed whitespace-pre-wrap ${
          ...
          {message.error && onRetry && (
            <div className="mt-1.5">
              <button
                onClick={onRetry}
                className="text-xs font-semibold text-rose-700 hover:text-rose-900 underline underline-offset-2"
              >
                Retry
              </button>
            </div>
      ...
      {!isUser && message.optimizeOffer && onOptimize && (
        <div className="ml-9 mt-2">
          <button
            ...
            disabled={sending}
            className="inline-flex items-center gap-1.5 text-xs font-semibold px-3 py-1.5 rounded-lg bg-brand-600 text-white hover:bg-brand-700 transition shadow-sm disabled:bg-slate-300 disabled:cursor-not-allowed"
          >
