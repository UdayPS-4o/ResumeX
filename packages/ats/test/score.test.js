...
test('buzzwords are flagged', () => {
  const res = runAtsChecks({
    summary: 'A hard-working team player and self-starter who is detail-oriented and passionate about delivering cutting-edge world-class results-driven solutions across every project for the whole team',
    experience: [{ bullets: ['Built a production system that processed user requests at scale'] }],
  });
  assert.ok(res.issues.some(i => /team player|cutting-edge/.test(i.text)), 'expected buzzwords surfaced in an issue');
});

test('first-person pronouns are flagged', () => {
  const res = runAtsChecks({
    summary: 'I am an engineer. I led my team and we shipped products. My focus is on building reliable systems that our customers rely on every single day across the world.',
    experience: [{ bullets: ['Built and shipped a production platform serving thousands of customers'] }],
  });
  assert.ok(res.issues.some(i => /first-person/i.test(i.text)), 'expected a first-person issue');
});

test('a complete but weakly-quantified resume is not over-scored', () => {
  // Every section, dates, and many skills present — but 0 quantified bullets,
  // first person, and a buzzword. Mainstream checkers land this in the 50s–low
  // 70s; our recalibrated engine must not inflate it to 80+ on completeness.
  const weak = {
    contact: { email: 'a@b.com', phone: '5551234', location: 'Indore, India' },
    summary: 'Full-stack developer focused on shipping production features and building automation systems. I enjoy digging into how systems work to deliver scalable solutions.',
    experience: [{
      company: 'Acme', title: 'Founding Engineer', start: 'Jan 2023', end: 'Feb 2025',
      bullets: [
        'Owned the end-to-end tech stack from architecture to dashboards and system integrations',
        'Collaborated with cross-functional teams to ensure seamless integration with existing systems',
        'Shipped production features and bug fixes across web and mobile to improve reliability',
      ],
    }],
    education: [{ school: 'IPS Academy', degree: 'BTech Computer Science', start: '2023', end: '2027' }],
    skills: [
      { category: 'Frontend', items: ['React', 'Next.js', 'TypeScript', 'Tailwind'] },
      { category: 'Backend', items: ['Node.js', 'Python', 'MySQL', 'MongoDB'] },
    ],
  };
  const res = runAtsChecks(weak);
  assert.ok(res.score < 75, `expected recalibrated score < 75, got ${res.score}`);
  assert.ok(res.score > 45, `expected a complete resume to still score > 45, got ${res.score}`);
  assert.equal(cat(res, 'Quantified impact').score, 0, 'no quantified bullets → 0 impact');
});
