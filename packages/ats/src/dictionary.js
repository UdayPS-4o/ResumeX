...
// ── De-buzzwording (debuzz.js) ──────────────────────────────────────────────
// Deterministic "weak/AI-flavored word → plainer word" replacements. Keys are
// single lowercase words (matched whole-word, any inflection handled by the
// caller is NOT done here — these are exact lemmas). Seeded from Resume-Matcher's
// refiner.py AI_PHRASE_REPLACEMENTS plus common résumé-coach swaps. Replacement
// preserves the original casing of the first letter (see debuzz.js).
export const AI_WORD_REPLACEMENTS = {
  // overused / AI-flavored action verbs → plain equivalents
  spearheaded: 'led',
  ...
  utilizing: 'using',
  utilization: 'use',
  // empty corporate adjectives → concrete-ish equivalents
...
// Multi-word AI/filler phrases to delete or simplify. Map value "" means strip
// the phrase entirely; a non-empty value replaces it. Matched whole-phrase,
// case-insensitively, longest-first (so "in light of the fact that" wins over
// "in order to"). Seeded from refiner.py's filler + buzzword phrase lists.
export const AI_PHRASE_REPLACEMENTS_RAW = {
  'best-in-class': 'top',
  'best in class': 'top',
  ...
  'a number of': 'several',
  'responsible for handling': 'handled',
  // strip-only filler (no useful replacement)
