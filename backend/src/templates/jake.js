...
export function renderJake(r, opts = {}) {
  ...
  if (r.summary) {
    blocks.summary = `\\section{${H('summary', 'Summary')}}\n${mdTex(r.summary)}`;
  }

  if (r.education?.length) {
    blocks.education = `\\section{${H('education', 'Education')}}
...
${r.education.map(eduItem).join('\n')}
  ...
  if (r.experience?.length) {
    blocks.experience = `\\section{${H('experience', 'Experience')}}
  \\resumeSubHeadingListStart
${r.experience.map(expItem).join('\n')}
  ...
  if (r.projects?.length) {
    blocks.projects = `\\section{${H('projects', 'Projects')}}
  \\resumeSubHeadingListStart
${r.projects.map(projItem).join('\n')}
  ...
  if (r.skills?.length) {
    blocks.skills = `\\section{${H('skills', 'Technical Skills')}}
...
\\end{itemize}`;
  ...
  if (r.certifications?.length) {
    blocks.certifications = `\\section{${H('certifications', 'Certifications')}}
  \\resumeSubHeadingListStart
...
${a.description ? `      \\resumeItemListStart\n        \\resumeItem{${mdTex(a.description)}}\n      \\resumeItemListEnd` : ''}`).join('\n')}
  ...
  return `\\documentclass[${paper},11pt]{article}
...
function eduItem(e) {
  return `    \\resumeSubheading
    ...
    e.details?.length
      ? `\n      \\resumeItemListStart\n${e.details.map(d => `        \\resumeItem{${mdTex(d)}}`).join('\n')}\n      \\resumeItemListEnd`
      : ''
  }`;
}
...
function expItem(x) {
  return `    \\resumeSubheading
    ...
    x.bullets?.length
      ? `\n      \\resumeItemListStart\n${x.bullets.map(b => `        \\resumeItem{${mdTex(b)}}`).join('\n')}\n      \\resumeItemListEnd`
      : ''
  }`;
}
...
function projItem(p) {
  ...
  return `    \\resumeProjectHeading
      {\\textbf{${tex(p.name || '')}}${techLine}${linkLine}}{${dateRange(p.start, p.end)}}${
    p.description
      ? `\n      \\resumeItemListStart\n        \\resumeItem{${mdTex(p.description)}}${
          (p.bullets || []).map(b => `\n        \\resumeItem{${mdTex(b)}}`).join('')
        }\n      \\resumeItemListEnd`
      : p.bullets?.length
        ? `\n      \\resumeItemListStart\n${p.bullets.map(b => `        \\resumeItem{${mdTex(b)}}`).join('\n')}\n      \\resumeItemListEnd`
        : ''
  }`;
}
